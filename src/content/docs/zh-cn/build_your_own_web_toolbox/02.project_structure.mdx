...
API_BASE_URL=https://apis.juhe.cn
...
4. 创建环境变量类型声明（`src/env.d.ts`）：

```typescript
/// <reference types="astro/client" />

interface ImportMetaEnv {
  readonly JUHE_API_KEY: string;
...
export async function GET({ params, request }) {
  const apiKey = import.meta.env.JUHE_API_KEY;
  const baseUrl = import.meta.env.API_BASE_URL;
  
  // 使用环境变量中的配置调用 API
  const response = await fetch(`${baseUrl}/weather?key=${apiKey}`);
