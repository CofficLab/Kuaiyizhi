...
export default class SidebarDB {
     ...
     * ]
     */
    static async getTopLevelSidebarItems(lang: string): Promise<SidebarItem[]> {
        const topLevelCategories = await ContentDB.getTopCategories();
     ...
     */
    static async getSidebarItemsByDocId(docId: string, level: number): Promise<SidebarItem[]> {
        const debug = false;
        if (debug) {
        ...
        const children = await ContentDB.getChildren(docId);
        for (const child of children) {
            // 判断当前child是否还需要继续获取子节点

            const childSidebarItems = await SidebarDB.getSidebarItemsByDocId(child.id, level - 1);
     ...
     */
    static async getSidebarItemByDocId(docId: string, level: number): Promise<SidebarItem> {
        // 去除docId两侧的斜杠
        ...
        const parentDocId = ContentDB.getParentDocId(parsedDocId);
        return SidebarDB.getSidebarItemByDocId(parentDocId, level - 1);
    }
     ...
     * @returns {Promise<SidebarItem>} 返回侧边栏项目
     */
    static async getSidebarItemBySlug(slug: string, level: number): Promise<SidebarItem> {
        const docId = ContentDB.slugToId(slug);
        return SidebarDB.getSidebarItemByDocId(docId, level);
    }
}